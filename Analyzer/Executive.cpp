...
{
	std::cout << "\n";
	std::cout << "-------------------------------REQUIREMENT 3:Created Executive instance----------------------------------\n";
...
{
	std::cout << "\n-------------------------------Requirement 8-Processing Command Line arguments:----------------------------------\n";
  ...
  try {
    ...
    for (int i = 2; i < argc; ++i)
    {
		...
		else if (std::strcmp(argv[i], "-XMLPathSCC") == 0)
			sccXmlPath = argv[i + 1];
		else if (std::strcmp(argv[i], "-XMLPathDep") == 0)
			depXmlPath = argv[i + 1];
    }
    if (patterns_.size() == 0)
    {
...
int main(int argc, char* argv[])
{
  ...
  try {
	  ...
	  //Calling display functions for each of
	  //typetable,dependancy table and strongly conneceted components
	  DisplayItems dpIt;
	  dpIt.displayTypeTable(tb);
	  std::cout << "\n";
	  std::cout << "\n";
	  dpIt.displayDependancyTable(dp);
	  std::cout << "\n";
	  std::cout << "\n";
	  dpIt.displayStrongComponents(scc);
