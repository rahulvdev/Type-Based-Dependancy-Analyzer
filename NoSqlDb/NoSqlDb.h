...
public:
	...
	~NoSqlDb()
	{
		std::cout << "**********************************USED MY OWN DATABASE************************************************\n";
		std::cout << "--------------REQUIREMENT 7:Writing Dependancy information in path: " << xmlFileName << " ------------\n";
		toXml(xmlFileName);
	}
...
//	}
//	else {
//		Element<Data> tempElem = value(key);
//		std::vector<std::string> childVect = tempElem.children;
//		std::vector<std::string> initVect = elem.children;
//		for (std::string childToAdd : initVect) {
//			for (std::string checkName : childVect) {
//			childVect.push_back(childToAdd);
//		}
//		tempElem.children = childVect;
//		store[key] = tempElem;
//	}
//	return true;
//}
